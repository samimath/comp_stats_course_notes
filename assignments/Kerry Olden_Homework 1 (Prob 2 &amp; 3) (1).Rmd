...
```{r, echo=FALSE}
spuRs::newtonraphson(dloglik, ddloglik, mme)
spuRs::newtonraphson(dloglik, ddloglik, -2.7)
spuRs::newtonraphson(dloglik, ddloglik, 2.7)
```
...
```{r, echo=FALSE}
mydata <- data.frame(Days = c(0,8,28,41,63,79,97,117,135,154),
...
model = nls(Beetles ~ SSlogis(Days,a,b,c), data = mydata)
model
#Graph
plot(mydata$Days, mydata$Beetles)
lines(mydata$Days, predict(model))
```
