...
dice.roll.g <-function(n){
  ...
  for (i in 1:n){
    ## over sample
    s <- c(s,sample(c(1,2,3,4,5,6),1,prob = c(0.5,0,0,1/6,1/6,1/6)))
  }
  ## correct by importance ratio
  exp.val <-(1/3)*sum(s==1)/n
  return(exp.val)
}
...
par(mfrow = c(2,4))
for(i in 1:length(sample.points)){
  hist(val[,i],xlim = c(0.1,0.4),
       main = paste(sample.points[i],'samples'),
       col = '#A93226',xlab = 'E(X)')
}
## visualize the result
par(mfrow = c(1,2))
plot(ns,f.est,type ='b',col = 'blue',ylim = c(0.1,0.25),
...
plot(ns,g.est,type ='b',col = 'forestgreen',ylim = c(0.1,0.25),
